    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('This Movie is not found in my database ,
                                          reason - Movie not released ,NO cam prints,HD files mathreme bot ill upload chyu vro .
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("I Like Your Smartness, But Don't Be Oversmart 😒updates chanel join cheythe evide kedan try again koduthu oru kariyuvumila😙", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
